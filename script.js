...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Inicializa Carrosséis de Professores
    const carouselProfessores1 = new Carousel('carousel-professores-1', 'carouselTrack-professores-1', 'carouselIndicators-professores-1');
    const carouselProfessores2 = new Carousel('carousel-professores-2', 'carouselTrack-professores-2', 'carouselIndicators-professores-2');
    const carouselProfessores3 = new Carousel('carousel-professores-3', 'carouselTrack-professores-3', 'carouselIndicators-professores-3');
...
// Funções do Popup Lattes
function openLattesPopup(element) {
    const popup = document.getElementById('lattesPopup');
    if (!popup) return;

    const name = element.getAttribute('data-name');
    const lattes = element.getAttribute('data-lattes');
    const imgElement = element.querySelector('img');
    const imgSrc = imgElement ? imgElement.src : '';

    const popupName = document.getElementById('lattes-popup-name');
    const popupLink = document.getElementById('lattes-popup-link');
    const popupImg = document.getElementById('lattes-popup-img');

    if (popupName) popupName.textContent = name;
    if (popupImg) popupImg.src = imgSrc;

    if (popupLink) {
        popupLink.href = lattes;

        // Opcional: Se não houver link (#), desabilitar ou avisar
        if (lattes === '#' || !lattes) {
            popupLink.onclick = (e) => {
                e.preventDefault();
                alert('Currículo Lattes ainda não adicionado.');
            };
            popupLink.style.opacity = '0.7';
            popupLink.style.cursor = 'not-allowed';
            popupLink.textContent = 'Lattes não disponível';
        } else {
            popupLink.onclick = null;
            popupLink.style.opacity = '1';
            popupLink.style.cursor = 'pointer';
            popupLink.textContent = '📄 Acessar Currículo Lattes';
        }
    }
